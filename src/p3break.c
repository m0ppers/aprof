...
UWORD            trap_fix_cmd;      // Assemblercode f�r Kommando
UWORD            trap_tmp_cmd;      // Assemblercode f�r Kommando
...
// Entfernt alle in breaktable eingeh�ngten Daten aus dem Speicher 
...
static BOOL SetTraps( void )
{
   ...
   // �bergabe der allokierten Traps an p3trap.asm
 ...
 * eingeh�ngt.
 * Am Programmeintritt mu� ein Symbol definiert sein, damit beim
 * Verlassen des Profilee mit exit() oder einem anderen nicht-lokalen
 * Sprung der Stack abger�umt wird und nicht der Fall auftritt, da� eine
 * Funktion zwar einen Hitcount von 1 hat, aber keine Ausf�hrungszeit
 * bestimmt wurde.
 */
static struct Symbol *CheckEntryBreak( struct Symbol *s, BPTR seg )
{
   ...
   /* Suchen eines Breakpoints f�r Programmeinsprung */
   ...
   /* count mu� im Bereich 0 <= count <= 2 liegen.
    *
    * Bei count == 0 mu� *ENTRY* eingef�gt werden,
...
BOOL BreakSet( void )
{
   ...
   for ( sy = THE_BASE.pb_symlist ; sy ; sy = sy->s_next )
   {
      ...
      // R�ckbezug von BreakPoint auf Symbol erstellen
      sy->s_break->fix.bp_symbol = sy;

      // Urspr�nglichen Befehl sichern
   ...
   // 68040 Unterst�tzung (I'm only dreaming...) Cache zur�ckschreiben
   CacheClearU();
...
/* Macht aus der Adresse eines BreakPoints das dazugeh�rige Symbol

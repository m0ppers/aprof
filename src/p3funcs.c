...
#define STX_BUFFER      256     /* Gr��e f�r statische Pufferbereiche */

static char buffer[ STX_BUFFER ];
static char fname[ STX_BUFFER ];   /* Puffer f�r Dateinamen */


#ifdef __MAXON__
/* strnicmp
 *
 * Vergleich zweier Strings bis zur L�nge n unabh�ngig von Gross- und
 * Kleinschreibung
 */
...
char *strins( char *s, int p, char *i )
{
    ...
    if ( !s || !i || p > strlen( s ) )      // Wenn p oder s NULL oder ...
        return( NULL );                     // Position au�erhalb

    taill = strlen( &s[p] ) +1;             // Inkl. EOS
    movel = strlen ( i );                   // Ohne EOS

    memmove( &s[p+movel], &s[p], taill );   // Bereiche �berlappen
    memcpy( &s[p], i, movel );
...
#if 0
// Diese Funktionen wurden wg. einem Bug im Maxon C++ qsort() eingef�hrt
   ...
   int (* cmp)(const void *, const void *) )
{
   ...
   if ( l < r )
   {
      ...
      for (;;)
      {
         // Solange von links die Elemente kleiner als Pivot sind, hochz�hlen
         while ( cmp( &k[i], &p ) < 0 )
            i++;

         // Solange von rechts Elemente gr��er als Pivot sind, runterz�hlen
         while ( cmp( &k[j], &p ) > 0 )
            j--;
         ...
         // x,y d�rfen nicht kleiner respektive gr��er als i,j werden
         if ( y < j )
            j = y;
         if ( x > i )
            i = x;
...
/****************************************************************************
 * Table2Idx - Sucht String in einer Tabelle und gibt den Index zur�ck
 */
...
/**************************************************************************
 * RequestFilenameW - holt Dateiname von User, Datei wird �berschrieben
 *
...
static BOOL save_data( FILE *file )
{
   ...
   /*
    * Die folgenden Stringkonstanten d�rfen keine Tabs enthalten
    */
...
/* men_save - Sichert Ergebnisse eines Profile-Laufs [Men�] File/Save
 *
...
/* pro_about - Gibt Versionsmeldung aus ([Men�] File/About)
 *
 * Eingabe: keine
 * Ausgabe; keine
 */

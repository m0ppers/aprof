 ...
 * $RCSfile: p3gui.c,v $
 *
 * Gadgetdefinitionen f�r das Hauptfenster
 *
...
// Die folgenden enum-Definitionen m�ssen parallel zur Liste laufen!
...
// Hilfsvariable f�r Aufbau und abschlie�ende Freigabe der Gadgetliste
static struct Gadget *glist;

// Zeiger auf Gadgets, die f�r SetAttr() ben�tigt werden
...
static void CalculateGadgetLayout( struct Window *win, APTR visual, struct NewGadget *gl )
{
   ...
   // Berechnung der Gadgeth�he in Abh�ngigkeit vom default font (1.5 * Zeilenh�he)
   ...
   // Anzeigebereich des Console.device einschr�nken
   DspSetTopOffset( crLineP + GadHi );

   /* Minimale Fensterh�he so einstellen, da� immer die

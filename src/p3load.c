 ...
 * $RCSfile: p3load.c,v $
 *
 * Laderoutinen f�r Profiler - Compilerunabh�ngiges Lesen von Symbolhunks
 *
 ...
 * Symboltabelle mu� aufsteigend geordnet sein
 */
static void OptSymList( void )
{
   ...
   /* R�ckkehr, falls keine Symbolliste existiert
    */
   if ( !src )
      return;
    ...
    * g�ltigen Symbole und bad Doppeleintr�ge
    * enth�lt.
    */
   while ( src )
   {
...
void men_open( void  )
{
   ...
   // Gesamtausf�hrungszeit l�schen
   UpdateXTime( NULL );
}



// Aufgelaufene Zeitwerte auf 0 setzen (File/reset im Men�)
void men_reset( void )
{
   struct Symbol *s;
   ...
   if ( THE_BASE.pb_symlist )
   {
      ...
      // Gesamtlaufzeit r�cksetzen
      THE_BASE.pb_base_time = 0;

      // Anzeige der Gesamtausf�hrungszeit l�schen
      UpdateXTime( NULL );
...
// TODO: Es sollte bin�re Suche verwendet werden!
//       Schliesslich laufen wir im Supervisor Mode

static ULONG SaveTable[256][2];


// Hier wird die Tabelle erzeugt, die das Save Profiling erm�glicht.
ULONG **CreateSaveTable( void )
{
   ...
   // Optimieren - d.h. Hunks mit Gr�sse 0 werden entfernt
   for ( i = 0 ; i < MaxHunk ; i++ )
   {
      if ( 0 == SaveTable[i][1] )
      {
...
/* Funktion l�uft im Supervisor Mode
 */
BOOL IsCodeHit( ULONG ret )
{
   ...
   // Sollte es n�tig sein, kann hier noch an der Position ~adr-2

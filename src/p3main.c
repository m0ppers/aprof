...
#define STX_LEFT        0   /* Konfigurierbar �ber WINDIM Tooltype */
#define STX_TOP         0
 ...
 *
 * Erledigt Programmabbruch f�r den Fall, da� AProf unter
 * einer BS Version < 2.04 gestartet wurde oder wichtige
 * Libraries nicht ge�ffnet werden konnten.
 *
...
/* CloseLibs
 *
 * Schliessen aller ge�ffneten Libraries.
 */
...
static BOOL OpenLibs( void )
{
    ...
    * wenn sie nicht ge�ffnet werden k�nnen
    */

   // Optionale Bibliothek f�r Hilfesystem
   if ( NULL == ( AmigaGuideBase = OpenLibrary( "amigaguide.library", MIN_AGUIDE_VER ) ) )
   {
      ...
      // Spricht ein Men�eintrag die Hilfefunktion an?
...
/* extract_name
 *
 * Wurde eine Datei �ber Kommandozeile, DefaultTool oder ExtendedSelect
 * gew�hlt, dann erfolgt R�ckgabe von Zeiger auf den Namen sonst NULL
 */
static char *extract_name( int c, char *v[] )
{
   ...
   if ( c )                        // CLI
   {
       ...
       * argv[1] kann nur g�ltig oder NULL sein
       */
      return v[1];
   }
   else                            // Workbench
   {
      ...
      {
         // �berspringe Argumente solange nicht wa_Lock und wa_Name
         // belegt sind
         for ( curarg = 1 ;
            (curarg < wbst->sm_NumArgs) && !wbag[curarg].wa_Lock && !wbag[curarg].wa_Name;
            ++curarg )
            ; // G�ltiges Argument suchen
...
/* DestroyCli
 *
 * Gibt Cli-Struktur frei. Wird von atexit ausgef�hrt.
 */
...
BOOL CreateCli( void )
{
      ...
      TAG_DONE ) )
   {
      ...
      Forbid();
         ...
         /* Cli-Struktur in Process-Struktur h�ngen */
      ...
      /* TODO: Bei CON: sollte zumindest ein Name f�r
...
/* wbmain
 *
 * Einsprung f�r Start von der Workbench f�r
 * Maxon C++
 */
#ifdef __MAXON__

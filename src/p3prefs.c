...
// Verschl�sselung der zus�tzlichen Gadget Information
 ...
 *      o EQ_QIT      Requester schlie�en, Daten nicht �bernehmen
 *      o EQ_USE      Requester schlie�en, Daten an Anwendung liefern
 */
...
/*
 * DlgPattern[]:
 *  Dieses Array enth�lt Beschreibungen f�r alle verwendeten Gadgets. Die
 ...
 *  UserData enth�lt einen Methodenaufruf f�r das jeweilige Gadget (wenn
...
/*
 * Berechnung der Gadget Positionen und -h�hen
 */
static ULONG DlgPatternLayout( struct NewGadget *gl )
{
   ...
   // Berechnung der Gadgeth�he in Abh�ngigkeit vom default font (1.25 * Zeilenh�he)
...
void men_sympat( void )
{
   ...
   // Tempor�re Variablen initialisieren
   mode = GetModeIdx();
   unit = GetEinheitIdx();
   sort = GetSortIdx();
   safe = GetSaveProf();

   // Gadgetpositionen und -h�hen berechnen
   ...
   if ( gad )
   {
      // Parent Window f�r Positionierung
   ...
   FreeGadgets( glist );       // Mu� immer durchlaufen werden!! (gad == NULL)
}

 ...
 * $RCSfile: p3timer.c,v $
 *
 * Handle-Modul f�r CIA-Timer
 *
 * Koppeln von Timer A & B zu einem 32-Bit-Z�hler
 * Z�hlfrequenz auf E-Takt einstellen (716 kHz = 1/10 ProTakt)
 * CIA(B)
 *
...
/* Fehlmessung f�r Aufruf von StartTimeMark und StopTimeMark
 */
...
/* TimeOver
 *
 * Routine wird bei Timeroverflow als Interrupt ausgef�hrt
 */
 ...
 *
 * R�ckgabe: Aktueller Stand des Timers
 *
 */
...
/* SetTimerCIA
 *
 * R�cksetzen des Timers auf Startwert
 *
 * Eingabe: Startwert
 */
 ...
 * Zeitnahme beenden
 *
 * R�ckgabe: Seit letztem StartTimeMarkCIA vergangene Ticks
 */
 ...
 *
 * R�ckgabe: Anzahl der Taskswitches zwischen InitTimer und ExitTimer
 */
 ...
 * Koppeln zu 32 Bit-Z�hler, Z�hlfrequenz E-Takt 716 Hz einstellen
 *
 * R�ckgabe: 0 = Timer nicht verf�gbar
...
BOOL InitTimerCIA( void )
{
   ...
   if ( (ciab.ciacra & CIACRAF_START) || (ciab.ciacrb & CIACRBF_START) )
      return FALSE;                    // => Timer nicht verf�gbar
   ...
   /* Switch und Launch Funktionen in Proze�struktur installieren */
   Disable();
      pro = (struct Process *)FindTask( NULL );
   ...
   /* Z�hlfrequenz Timer A auf E-Takt stellen (ca. 716 kHz)          */
   ciab.ciacra &= ~CIACRAF_INMODE;       /* %1101 1111 > l�schen     */
   ...
   /* RUNMODE = continous */
   ciab.ciacra &= ~CIACRAF_RUNMODE;      /* l�schen  */
   ciab.ciacrb |=  CIACRBF_RUNMODE;      /* setzen   */

   /* Startwerte der Z�hler initialisieren (0xffffffff) */
   SetTimerCIA( (ULONG)-1 );
